...
\question{We know that the overall round trip minimum is $1$s. We can use Christian's algorithm to determine the round trip time we must receive, in order to achieve the desired accuracy range of $2$s. accuracy\_range = $T_{round} - T_{round\_min}$. Which substituting the appropriate values we get $T_{round} = 2+1 = 3$s. \textbf{Note here that the question states the minimum round trip time is $1$ second, this does not infer a symmetric relationship between the two point to point trips.} }
